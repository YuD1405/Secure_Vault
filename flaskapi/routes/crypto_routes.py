...
from modules.crypto.key_management import get_all_key_strings, check_and_manage_own_keys, regenerate_key_handling, extend_key_expiry
...
def manage_keys():
    ...
    try:
        check_and_manage_own_keys(email, passphrase, key, salt)
...
def regenerate_key():
    ...
    try:
        ...
        success = regenerate_key_handling(email, passphrase, aes_key, salt)
        if success:
            log_user_action(email, "Regenerate RSA Key", "Success", "New RSA key + recovery encrypted")
            return {"success": True, "message":  "Key pair regenerated successfully."}
        else:
            raise Exception("Failed to regenerate key")
    except Exception as e:
        log_user_action(email, "Regenerate RSA Key", "Fail", f"{e}", level="error")
        return {"success": False, "message": f"Regeneration error: {e}"}, 500

@crypto_bp.route('/extend_key', methods=['POST'])
def api_extend_key():
    data = request.get_json()
    email = session["email"]
    days_to_add = data.get("days_to_add")

    if not email or not isinstance(days_to_add, int):
        log_user_action(email or "unknown", "Extend Key", "Failed", "Missing email or invalid days_to_add", "warning")
        return jsonify({"success": False, "message": "Missing email or invalid number of days."}), 400

    success, message = extend_key_expiry(email, days_to_add)
    log_user_action(email, "Extend Key", "Success", f"Extend Key by {days_to_add} days")
